...
<body>
  <?php
  $editName = $editEmail = $editPass = $editConfirmP = $editPhone = $editIG = $editBio = $editBalance = $editStatus = $editPermission = "";
  ...
  if (isset($_POST["editUser"])) {
    ...
    $editBalance = filterInput($_POST["editBalance"]);
    $editStatus = filterInput($_POST["editUserStatus"]);
    $editPermission = filterInput($_POST["editUserPermission"]);

    if ($editName != "" && $editEmail != "" && $editPhone != "" && $editStatus != "" && $editPermission != "") {
      if (isValidEmail($editEmail)) {
        if (isValidPhone($editPhone)) {
          if (isValidName($editName)) {
            if ($editPass == $editConfirmP) {
              if (isValidStatus($editStatus)) {
                if (isValidPermission($editPermission)) {
                  ...
                  function_alert("Please select a permission for the account");
                }
              } else {
                function_alert("Please select a status for the account");
              }
            } else {
            ...
            <div class="d-flex justify-content-center">
              <section class="scroll-section" id="title">
                <div class="page-title-container">
                  <h1 class="mb-0 pb-0 display-4">Edit User Profile</h1>
                </div>
              </section>
            </div>
            <!-- Title End -->
                  ...
                  <form class="card mb-9 tooltip-end-top" id="signUpForm" method="POST" novalidate>
                    <div class="card-body">
                      ...
                      <div class="mb-4 filled">
                        ...
                        <input class="form-control" type="text" placeholder="Current balance credit for class bookings" name="editBalance" value="<?php echo $userDBPackageBal; ?>" />
                      </div>
                      <div class="input-group mb-3">
                        <label class="input-group-text" for="inputGroupSelect01">User Status</label>
                        <select class="form-select" id="inputGroupSelect01" name="editUserStatus">
                          <option value="">Please select an User Status</option>
                          <option value="1">Active</option>
                          <option value="0">Inactive</option>
                        </select>
                      </div>
                      <div class="input-group mb-3">
                        <label class="input-group-text" for="inputGroupSelect01">User Permission</label>
                        <select class="form-select" id="inputGroupSelect01" name="editUserPermission">
                          <option value="">Please select User Permission</option>
                    ...
                    <div class="card-footer border-0 pt-0 d-flex justify-content-between align-items-center">
                      <div>
                        <button class="btn btn-icon btn-icon-end btn-primary" name="editUser" type="submit">
